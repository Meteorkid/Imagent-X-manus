    ...
    public String createAndStartContainer(String containerName, ContainerTemplate template, Integer externalPort,
        ...
        try {
            ...
            HostConfig hostConfig = HostConfig.newHostConfig().withBinds(bind)
                    ...
                    // 沙箱安全隔离配置
                    .withSecurityOpts(Arrays.asList("no-new-privileges:true")) // 禁止特权提升
                    .withCapDrop(Capability.ALL) // 移除所有Linux capabilities
                    .withCapAdd(Capability.CHOWN, Capability.SETGID, Capability.SETUID) // 仅允许必要权限
                    .withReadonlyRootfs(true) // 只读文件系统
                    // .withTmpfs(Collections.singletonMap("/tmp", "noexec,nosuid,size=100m")) // 临时文件系统 - 暂时注释掉
                    .withUlimits(Arrays.asList(
                        new Ulimit("nofile", 1024L, 2048L), // 文件描述符限制
                        new Ulimit("nproc", 100L, 200L)     // 进程数限制
                    ))
                    .withPidsLimit(50L); // 最大进程数限制
