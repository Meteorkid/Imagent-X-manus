...
# ImagentX.imagentx.top Cloudflare部署脚本
...
echo -e "${BLUE}☁️ ImagentX.imagentx.top Cloudflare部署脚本${NC}"
echo "=================================="
...
# 检查A记录是否存在
A_RECORD_EXISTS=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records?name=imagentx.top&type=A" \
    -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" \
...
if [ "$A_RECORD_EXISTS" -eq 0 ]; then
    ...
    curl -s -X POST "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records" \
        ...
        --data "{\"type\":\"A\",\"name\":\"imagentx.top\",\"content\":\"$SERVER_IP\",\"ttl\":1,\"proxied\":true}" > /dev/null
    echo -e "${GREEN}✅ 创建A记录: imagentx.top -> $SERVER_IP${NC}"
else
    # 更新A记录
    RECORD_ID=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records?name=imagentx.top&type=A" \
        -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" \
    ...
    curl -s -X PUT "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records/$RECORD_ID" \
        ...
        -H "Content-Type: application/json" \
        --data "{\"type\":\"A\",\"name\":\"imagentx.top\",\"content\":\"$SERVER_IP\",\"ttl\":1,\"proxied\":true}" > /dev/null
    echo -e "${GREEN}✅ 更新A记录: imagentx.top -> $SERVER_IP${NC}"
fi

# 配置www子域名
WWW_RECORD_EXISTS=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records?name=www.imagentx.top&type=A" \
    -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" \
    -H "Content-Type: application/json" | jq -r '.result | length')
...
if [ "$WWW_RECORD_EXISTS" -eq 0 ]; then
    curl -s -X POST "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records" \
        -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" \
        -H "Content-Type: application/json" \
        --data "{\"type\":\"A\",\"name\":\"www.imagentx.top\",\"content\":\"$SERVER_IP\",\"ttl\":1,\"proxied\":true}" > /dev/null
    echo -e "${GREEN}✅ 创建A记录: www.imagentx.top -> $SERVER_IP${NC}"
else
    WWW_RECORD_ID=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records?name=www.imagentx.top&type=A" \
        -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" \
        -H "Content-Type: application/json" | jq -r '.result[0].id')
    ...
    curl -s -X PUT "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records/$WWW_RECORD_ID" \
        ...
        -H "Content-Type: application/json" \
        --data "{\"type\":\"A\",\"name\":\"www.imagentx.top\",\"content\":\"$SERVER_IP\",\"ttl\":1,\"proxied\":true}" > /dev/null
    echo -e "${GREEN}✅ 更新A记录: www.imagentx.top -> $SERVER_IP${NC}"
fi
...
docker-compose -f docker-compose.imagentx.top.yml up -d
...
for i in {1..30}; do
    if curl -s http://imagentx.top > /dev/null 2>&1; then
...
echo "=================================="
echo -e "${CYAN}🌐 网站地址: https://imagentx.top${NC}"
echo -e "${CYAN}🔧 API地址: https://imagentx.top/api${NC}"
echo -e "${CYAN}🔍 健康检查: https://imagentx.top/health${NC}"
echo -e "${CYAN}📊 RabbitMQ管理: http://$SERVER_IP:15672${NC}"
echo ""
...
echo -e "${YELLOW}   - 查看日志: docker-compose -f docker-compose.imagentx.top.yml logs${NC}"
echo -e "${YELLOW}   - 停止服务: docker-compose -f docker-compose.imagentx.top.yml down${NC}"
echo -e "${YELLOW}   - 重启服务: docker-compose -f docker-compose.imagentx.top.yml restart${NC}"
echo ""
echo -e "${GREEN}✨ 现在可以通过 https://imagentx.top 访问您的ImagentX服务了！${NC}"
