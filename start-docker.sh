...
echo -e "${BLUE}🐳 Imagent X Docker 服务启动脚本${NC}"
echo "=================================="
...
if [ ! -f "working-docker-compose.yml" ]; then
    echo -e "${RED}❌ 配置文件 working-docker-compose.yml 不存在${NC}"
    exit 1
fi
...
echo -e "  - 使用 'docker-compose -f working-docker-compose.yml restart' 重启服务"
